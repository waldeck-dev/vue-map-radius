...
- **MapLibre 6 needs `setWorkerUrl()` from the host app**, or the map renders blank with no error: it locates its worker through its own `import.meta.url`, which no bundler resolves to the real file. The playground does it in `docs/main.ts` with `maplibre-gl/dist/maplibre-gl-worker.mjs?worker&url` — `?worker&url` and never a bare `?url`, which omits the sibling `maplibre-gl-shared.mjs` and breaks tile parsing in the build only. `vite.config.ts` additionally excludes `maplibre-gl` from `optimizeDeps` (dev server only; pre-bundling relocates that `import.meta.url`). The library itself never calls `setWorkerUrl()` — maplibre-gl is external, so the choice belongs to the consumer, and README documents it.
